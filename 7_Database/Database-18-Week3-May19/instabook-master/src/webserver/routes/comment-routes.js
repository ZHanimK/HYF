...
// Function that returns first name and last name of user as well as all the comments columns (comment-table.js)
commentRouter.get('/all/comments', async (req, res, next) => {
  try {
    console.log('test')
    const comments = await commentTable.getUserComments();
    return res.json(comments);
  } catch (err) {
    return next(err);
  }
